...
The mk2Wifi-C6 board adds wireless connectivity to the Mk2 PV Router using an **ESP32-C6-MINI-1** module (RISC-V single-core, WiFi 6, Bluetooth LE 5, Zigbee, Thread). It plugs directly into the mainboard via the TRIG_EXT and UART_EXT pin headers and is powered by +5V from the mainboard, regulated locally to +3.3V.

Key features:
- WiFi 802.11 b/g/n/ax (2.4 GHz) and Bluetooth LE 5
...
| File | Description |
|------|-------------|
| `mk2Wifi-C6.kicad_pro` | KiCad 9 project file |
| `mk2Wifi-C6.kicad_sch` | Schematic |
| `mk2Wifi-C6.kicad_pcb` | PCB layout |
| `mk2Wifi-C6.kicad_dru` | Design rules |
| `mk2Wifi-C6.kicad_sym` | Board-local custom symbol library |
...
| R5 | 1K | 0402 | D5 series protection (GPIO0) |
| R6 | 1K | 0402 | D6 series protection (GPIO5) |
| R7 | 1K | 0402 | D7 series protection (GPIO4) |
| R8 | 1K | 0402 | D8 series protection (GPIO3) |
| R9 | 1K | 0402 | D9 series protection (GPIO1) |
| R10 | 5K1 | 0402 | USB CC1 pull-down |
| R11 | 5K1 | 0402 | USB CC2 pull-down |
...
|------|-----|----------|-------|
| GPIO0 | 12 | D5 (trigger output) | 1K series resistor (R5) to TRIG_EXT pin 5 |
| GPIO1 | 13 | D9 (trigger output) | 1K series resistor (R9) to TRIG_EXT pin 6 |
| GPIO3 | 6 | D8 (trigger output) | 1K series resistor (R8) to TRIG_EXT pin 2 |
| GPIO4 | 9 | D7 (trigger output) | 1K series resistor (R7) to TRIG_EXT pin 3 |
| GPIO5 | 10 | D6 (trigger output) | 1K series resistor (R6) to TRIG_EXT pin 4 |
| GPIO6 | 15 | SDA (I2C data) | Direct to OLED pin 4 |
...
In normal operation, **+5V** is supplied from the mainboard through the UART_EXT header (pin 3). The AP2112K-3.3 LDO (U2) regulates this to +3.3V for the ESP32-C6 and OLED display, with a maximum output of 600mA.
...
The USB-C connector can also supply +5V during initial programming when the board is not connected to the mainboard. R10 and R11 (5K1 each) on CC1/CC2 configure the USB-C port as a UFP (sink) to request 5V from the host.
...
- The I2C bus (SCL/SDA) is **local to the mk2Wifi board only** -- it connects the ESP32-C6 to the OLED display and is not routed to the mainboard

## Programming

1. **Initial firmware upload**: Unplug the board from the mainboard, then connect a USB-C cable. The ESP32-C6 has a built-in USB-serial/JTAG controller -- no external programmer is needed.
